...
func progress_next_stage():
	#print("progr")
	match OldGlobals.current_gameplay_stage:
		OldGlobals.Gameplay_Stages.STAGE_0:
			reset_machine(2.5)
			OldGlobals.gameplay_stage_changed.emit()
			OldGlobals.current_gameplay_stage += 1
		OldGlobals.Gameplay_Stages.STAGE_1:
			reset_machine(2)
			OldGlobals.gameplay_stage_changed.emit()
			OldGlobals.current_gameplay_stage += 1
		OldGlobals.Gameplay_Stages.STAGE_2:
			reset_machine(2)
			OldGlobals.gameplay_stage_changed.emit()
			OldGlobals.current_gameplay_stage += 1
		OldGlobals.Gameplay_Stages.STAGE_3_FINAL:
